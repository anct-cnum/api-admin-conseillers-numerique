...
describe('recevoir et valider une candidature structure coordinateur', () => {
  ...
  it.each(['COORDINATEUR', 'CONSEILLER_ET_COORDINATEUR'])('si j’envoie un formulaire avec une missionCoordinateur égale à +%d alors il est validé', async (missionCoordinateur) => {
    // GIVEN
    const envoiUtilisateur = {
      ...champsObligatoires,
      missionCoordinateur,
  ...
  it.each([
      ...
      'COLLECTIVITE',
      'GIP',
      'PRIVATE',])('si j’envoie un formulaire avec une structure de type égale à +%d alors il est validé', async (type) => {
    // GIVEN
    const envoiUtilisateur = {
      ...champsObligatoires,
      type,
