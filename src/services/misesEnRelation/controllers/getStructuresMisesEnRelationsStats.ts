...
const getStructuresMisesEnRelationsStats =
  (app: Application) => async (req: IRequest, res: Response) => {
    try {
      ...
      const misesEnRelationFinalise = await app.service(service.misesEnRelation).Model.find({ 'statut': 'finalisee', 'structure.$id': structureId });
      ...
      const misesEnRelationValide = await app.service(service.misesEnRelation).Model.find({ 'statut': 'recrutee', 'structure.$id': structureId });
      const candidatsValide = misesEnRelationValide.map((item: any) => {
        return item.conseillerObj;
      });
      res.send(statsDisponibles.map((item: any) => {
