...
const getStatsGlobalesGrandReseau = async (
  query,
  codesPostauxQuery,
  ability,
  action,
  ...
  try {
    ...
    const codesPostaux = await getCodesPostauxGrandReseau(
      codesPostauxQuery,
      ability,
      action,
      app,
    ...
    const structures = await getStructures(query, ability, action, app);

    const conseillers = await getConseillers(query, ability, action, app);
...
export default getStatsGlobalesGrandReseau;
